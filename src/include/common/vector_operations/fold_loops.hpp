...
template <class LEFT_TYPE, class RESULT_TYPE, class OP, bool HAS_SEL_VECTOR>
static inline bool inplace_fold_loop_function(LEFT_TYPE *__restrict ldata, RESULT_TYPE *__restrict result, index_t count,
                                      sel_t *__restrict sel_vector, nullmask_t &nullmask) {
	ASSERT_RESTRICT(ldata, ldata + count, result, result + 1);
	if (nullmask.any()) {
		// skip null values in the operation
		...
		for (i++; i < count; i++) {
			index_t index = HAS_SEL_VECTOR ? sel_vector[i] : i;
			if (!nullmask[index]) {
				OP::Operation(*result, ldata[index]);
			}
		}
	} else {
		// quick path: no NULL values
		*result = ldata[HAS_SEL_VECTOR ? sel_vector[0] : 0];
		for (index_t i = 1; i < count; i++) {
			OP::Operation(*result, ldata[HAS_SEL_VECTOR ? sel_vector[i] : i]);
		}
	}
	return true;
}

template <class LEFT_TYPE, class RESULT_TYPE, class OP> bool templated_unary_inplace_fold(Vector &input, RESULT_TYPE *result) {
	auto ldata = (LEFT_TYPE *)input.data;
	if (input.sel_vector) {
		return inplace_fold_loop_function<LEFT_TYPE, RESULT_TYPE, OP, true>(ldata, result, input.count,
		                                                                    input.sel_vector, input.nullmask);
	} else {
		return inplace_fold_loop_function<LEFT_TYPE, RESULT_TYPE, OP, false>(ldata, result, input.count,
		                                                                     input.sel_vector, input.nullmask);
	}
}
